...
export default function injectShaderSplattingMap(targetMaterial: THREE.MeshStandardMaterial, textureLoader: TextureLoader, textureScaling = 15.0) {
    // set 2
    ...
    targetMaterial.onBeforeCompile = (shader) => {
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            dedent`
                ...
                vec3 mapMQ = mixDepthBased(vec4(mapM1,1.0-M2_Fac),vec4(mapM2,M2_Fac));
                
                vec3 mapM3 =  textureNoTile(uMap3, uNoiseFine, scaledUV);
                
                vec3 mapMX= mixDepthBased(vec4(mapMQ,1.0-M3_Fac),vec4(mapM3,M3_Fac));
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            // only tangent space normal map will work.
                ...
                vec3 normalMQ = mixDepthBased(vec4(normalM1,1.0-M2_Fac),vec4(normalM2,M2_Fac));
                
                vec3 normalM3 =  textureNoTile(uNormalMap3, uNoiseFine, scaledUV);
                
                vec3 normalMX = mixDepthBased(vec4(normalMQ,1.0-M3_Fac),vec4(normalM3,M3_Fac));
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            dedent`
                    ...
                    vec3 AoMQ = mixDepthBased(vec4(AoM1,1.0-M2_Fac),vec4(AoM2,M2_Fac));
                    
                    vec3 AoM3 = textureNoTile(uAoMap3, uNoiseFine, scaledUV);
                    
                    float AoMX = mixDepthBased(vec4(AoMQ,1.0-M3_Fac),vec4(AoM3,M3_Fac)).r;
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            float roughnessFactor = roughness;
                ...
                vec3 RoMQ = mixDepthBased(vec4(RoM1,1.0-M2_Fac),vec4(RoM2,M2_Fac));
                
                vec3 RoM3 =  textureNoTile(uAoMap3, uNoiseFine, scaledUV);
                
                vec3 RoMX = mixDepthBased(vec4(RoMQ,1.0-M3_Fac),vec4(RoM3,M3_Fac));
...
function makeTextureRepeat(texture: THREE.Texture, repeatTimes = 0) {
    if (repeatTimes > 0)
